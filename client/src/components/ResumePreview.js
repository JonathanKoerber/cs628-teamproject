...
const ResumePreview = ({ 
  ...
  return (
    <div>
      ...
      {/* Resume Preview */}
      <div id="resume-preview-content" className={`resume-preview template-${template}`}>
        ...
        <div className="resume-contact">
          ...
          {linkedin && <p>LinkedIn: <a href={linkedin} target="_blank" rel="noopener noreferrer">{linkedin}</a></p>}
          {github && <p>GitHub: <a href={github} target="_blank" rel="noopener noreferrer">{github}</a></p>}
        </div>
            ...
            {experience.map((exp, index) => (
              exp.title ? (
                <div key={index} className="resume-entry">
                  ...
                  <p>{exp.location} | {exp.start_date} - {exp.end_date}</p>
                  <ul>
          ...
          <div className="resume-section">
            ...
            <ul>
              {certifications.map((cert, index) => (
                cert.name ? <li key={index}>{cert.name} ({cert.year})</li> : null
              ))}
            </ul>
          </div>
        )}
